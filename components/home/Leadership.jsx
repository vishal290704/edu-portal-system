...
export default function Leadership() {
  const foundingPrincipal = leadership.find((person) => person.isLegacy);

  const currentLeaders = leadership.filter((person) => !person.isLegacy);
          ...
          <div className="rounded-3xl border border-amber-200 bg-amber-50 p-8 shadow-sm transition hover:shadow-xl">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="mt-4 max-w-lg text-center text-sm leading-7 text-slate-500">
                Forever remembered as the Founding Principal whose vision,
                dedication, and commitment laid the foundation of Dynamic
                English School and continue to inspire every generation of
                learners.
              </p>

              <blockquote className="mt-8 max-w-xl text-lg italic leading-8 text-slate-600">
                "{foundingPrincipal.message}"
              </blockquote>

              <Button variant="ghost" className="mt-8 text-[#0F4C81]">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          {currentLeaders.map((person) => (
            ...
            >
              ...
              <Button variant="ghost" className="mt-6 px-0 text-[#0F4C81]">
                {person.button}
